# Instructions: `align-test-patterns`
...
3. Use the `render-template` skill with the `.agents/domains/plans/templates/instructions-report.tart` to render your report and write it next to this instruction file: `plan-test-coverage/instructions/align-test-patterns__report.md`. No separate delegation record is created.
...
Align all tests to a common setup/lock => execute => assert pattern so the suite reads consistently.

## Mandatory Reading

- All test files across `$PARSER`, `$PRIMITIVES`, `$SERIALIZER`, `$CONSTRUCTS` — the tests to align.
...
### Step 1 of 5 — Review all tests for patterns

1. Review all test files across the 4 packages.
2. Identify the setup/lock => execute => assert patterns in use.

### Step 2 of 5 — Identify the pattern variants

1. Identify the variants: different setup steps, semantics, variable names, mock content, assertion order and detail for the same kind of test.

### Step 3 of 5 — Align the tests

1. Make tests more similar by aligning setup steps, semantics, variable names, mock content, assertion order and detail.
2. RULE: alignment must not change what the tests assert — only how they express it.

### Step 4 of 5 — Commit `align-test-patterns`
...
- Verify that tests of the same kind now read consistently (setup steps, semantics, variable names, mock content, assertion order and detail).
- Verify that no test changed what it asserts.
