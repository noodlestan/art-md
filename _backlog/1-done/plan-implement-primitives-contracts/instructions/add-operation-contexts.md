...
- Design attachment (authoritative source code + file locations): `$PROJECT/_roadmap/1-done/milestone-art-codec/milestone__design.md` — read `## Contracts` → `### ParseContext (in @art-md/primitives under parser/context/)` and `### SerializeContext (in @art-md/primitives under serializer/context/)`.
