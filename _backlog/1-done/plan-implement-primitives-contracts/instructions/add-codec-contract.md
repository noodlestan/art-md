...
- Design attachment (authoritative source code + file locations): `$PROJECT/_roadmap/1-done/milestone-art-codec/milestone__design.md` — read `## Contracts` → `### ArtCodec`, `### ParseResult / SerializeResult`, and `### ParserVisitContext (in @art-md/primitives under parser/context/)`.
