...
Create `$BUILD/cli/bin/src/private/config/types.ts` with `BinConfig` containing `output.mode` and `codec` overrides.

Create `$BUILD/cli/bin/src/private/config/loadBinConfig.ts` reading version and defaults from `package.json`, with optional user overrides. The bin does not own a config file format.

### Step `2 / 7` — Add codec context factory

Create `$BUILD/cli/bin/src/private/context/createCodecContext.ts`:
...
Create `$BUILD/cli/bin/src/private/io/readInput.ts` and `$BUILD/cli/bin/src/private/io/writeOutput.ts`:
...
Create `$BUILD/cli/bin/src/private/present/presentDocument.ts` and `$BUILD/cli/bin/src/private/present/presentContent.ts`:
...
Create `$BUILD/cli/bin/src/test/helpers/`:
